...
from networks import Actor, Critic,PolicyNeuralNetTorchWithAttention
...
def policy_train_single(relation, mode="PPO", task="", log_save_path="",extra_title="",wandb_show=False, 
                        project_name="deep-path",max_episode_num = 300,max_test_num = 500):
    ...
    retrain_checkpoint_path = os.path.join(
        base_path, 'torchmodels', f'retrain_{mode}_checkpoint_{relation}.pth')
    ...
    def PPO_train(training_pairs, actor_model, actor_optimizer, num_episodes,gap=None):
        ...
        for i_episode in range(num_episodes):
            ...
            env.reset_kb_list_by_task(train[i_episode])
    ...
    def retrain():
        ...
        training_pairs = env.get_train_data()
        print(f"Training data loaded {len(training_pairs)}")
        
        # 加载PPO Actor模型和优化器
        # 创建actor模型和优化器
        # actor_model = Actor().to(device)
        actor_model = PolicyNeuralNetTorchWithAttention().to(device)
        ...
        torch.save(retrain_checkpoint, retrain_checkpoint_path)
    ...
    def test(gap = 50):
        # 其实这里也不算是test，第一次为了找到路径去训练，第二次为了防止路径重复来训练，论文作者居然也在这里更新model了。。。
        # 还是同一批数据...
        ...
        if wandb_show:
            wandb.init(project=project_name, name=f"TEST_{mode}_{filter_relation}{extra_title}",config=hyperparameters)
        ...
        retrain_checkpoint = torch.load(retrain_checkpoint_path)
        actor_model = Actor().to(device)
        actor_model.load_state_dict(retrain_checkpoint['state_dict'])
        actor_optimizer = torch.optim.Adam(actor_model.parameters(), lr=learning_rate)
        actor_optimizer.load_state_dict(retrain_checkpoint['optimizer'])

        print("Retrained ppo model loaded")
        ...
        for episode in range(test_num):
            ...
            env.reset_kb_list_by_task(test_data[episode])
            
            
            # 获取初始状态索引
            state_idx = [env.entity2id[sample[0]],
                        env.entity2id[sample[1]], 0]
            ...
            if done:
                if path_set:
                    ...
                    # 记录的是当前步骤的损失值
                    if wandb_show:
                        wandb.log({"test actor_loss": actor_loss.item()})
