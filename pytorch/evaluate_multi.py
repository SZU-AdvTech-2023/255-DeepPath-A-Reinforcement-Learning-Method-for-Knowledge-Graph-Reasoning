...
from utils import relations, dataPath,seed_number,set_seed
import os
...
def evaluate_single(relation, dataset="NELL-995"):
...
if __name__ == '__main__':
    ...
    result_path = os.path.join(
        base_path, f"{dataset}_link_prediction_rl_map.csv")
    # 若不存在results文件夹，则创建
    ...
    for relation in relations:
        ...
        rl_map = evaluate_single(relation, dataset)
    ...
    print(f"Average RL MAP: {aver / count}")
    with open(result_path, "a") as f:
        f.write(f"Average RL MAP,{aver / count}\n")
        f.close()
