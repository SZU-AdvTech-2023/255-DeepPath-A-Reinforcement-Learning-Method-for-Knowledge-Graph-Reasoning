...
def fact_prediction_single(mode="RL", relation="concept_agentbelongstoorganization"):

    # 文件路径设置
    base_path = os.path.abspath(__file__)
    base_dir = os.path.dirname(base_path)
    dataset_name = "NELL-995"
    dataPath_ = os.path.join(base_dir, "..", dataset_name, 'tasks', relation)
    featurePath = get_path_to_use_path(mode, relation)
    feature_stats = get_path_stats_path(mode, relation)

    relationId_path = os.path.join(
        base_dir, "..", dataset_name, 'relation2id.txt')
...
if __name__ == "__main__":
    for relation in relations:
        ...
        fact_prediction_single(mode="RL", relation=relation)
