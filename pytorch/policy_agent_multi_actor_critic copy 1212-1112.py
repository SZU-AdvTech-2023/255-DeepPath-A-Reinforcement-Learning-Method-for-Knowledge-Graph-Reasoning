...
def policy_train_single(relation, mode="AC", task="test", log_save_path="",wandb_show = True):
    # 改为os.path.join
    ...
    def ac_train(training_pairs, actor_model, critic_model,
        ...
        if wandb_show:
            wandb.init(project="deep-path", name=f"{mode}_Training_filter_relation" )
        ...
        for i_episode in range(num_episodes):
            ...
            if wandb_show:
                wandb.log({
                    ...
                    'Average Path Length': all_length / success,
    ...
    def test():
        ...
        print(f"Start {mode} training {filter_relation}")
        if wandb_show:
            wandb.init(project="deep-path", name=f"{mode}_TEST_{filter_relation}")
    ...
    if task == 'test':
        time1,_ = measure_time(origin_test)
        time2,_ = measure_time(test)
        print("origin test time:", time1,"秒")
        print("test time:", time2,"秒")
...
#             future = executor.submit(policy_train_single, relation, mode, task, log_save_path,False)
